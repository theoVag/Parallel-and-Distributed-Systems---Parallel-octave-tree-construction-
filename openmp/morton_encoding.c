...
int i;  
#pragma omp parallel for private(i) shared(mcodes,codes) num_threads(num_th)//parallel for with variable i private for each thread and other shared variables for all threads and the number of threads=num_th
  for(i=0; i<N; i++){
