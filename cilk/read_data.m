function [results] = read_datalast(src, threads,nN,iter)
...
hash_mean = reshape(hash_mean,threads, nN);
...
morton_mean = reshape(morton_mean,threads, nN);
...
sort_mean = reshape(sort_mean,threads, nN);
...
dataR_mean = reshape(dataR_mean,threads, nN);

all_time=(hash_time+morton_time+sort_time+dataR_time);
all_mean = mean(reshape(all_time, iter, length(all_time) / iter), 1);
all_mean = reshape(all_mean,threads, nN);
