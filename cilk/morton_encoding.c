...
void morton_encoding(unsigned long int *mcodes, unsigned int *codes, int N, int max_level){
	...
	if (__cilkrts_set_param("nworkers",num_threads)!=0){//set number of workers
		//printf("Failed to set worker count\n"); disable in order to measure times corerctly
	}  
	__cilkrts_init();//initialize cilk

	//int numWorkers = __cilkrts_get_nworkers(); //only for testing
	//printf("We changed the number of workers in morton to %d.\n",numWorkers);
